...
- Honeycomb: a centered cluster of equal, edge-to-edge hexagons for three or more videos, with background around the outer boundary. One or two videos use the existing Hexagons behavior. Set its layout weight above zero to enable it in saved settings; playback and export both support it. Crop/fit still controls video framing inside each hexagon.
- Inset: one full-screen background video behind Circles, Hexagons, or Honeycomb, chosen equally from the shapes supported by the current count, independently of their layout weights. The background counts toward the configured total and always crops to fill; the existing Crop / Fit setting controls foreground videos, with Fit revealing the background through unused space. Honeycomb requires four total videos. One video plays full-screen. Clip timing, duplicate rules, and single-source audio apply to all layers in playback and export. Enable Inset through its layout weight.
- Presets support Load, Save, Save as, Delete, and Set default. Save confirms replacement and saves/applies the settings shown, including unapplied edits; Save as also confirms name collisions. Set default uses the saved preset at startup without changing the current session. Delete confirms, and deleting the loaded preset loads the next in the list (the previous if last). The final preset cannot be deleted. Deleting the default clears the startup choice and restores last-used settings on subsequent launches.
